...
public class WalkerGenerator : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        floor = GameManager.instance.levelDataInstance.floor;
        wall = GameManager.instance.levelDataInstance.wall;
    ...
    void SpawnPlayer()
    {
        ...
        while (!spawned)
        {
            Vector2 randomLocation = new Vector2(Random.Range(0, grid.GetLength(0)), Random.Range(0, grid.GetLength(1)));
            if (grid[(int)randomLocation.x, (int)randomLocation.y] != Grid.Floor) { continue; }

            playerInstance = Instantiate(player, new Vector2(randomLocation.x + 0.5f, randomLocation.y + 0.5f), Quaternion.identity);
            playerInstance.Init(GameManager.instance.selectedPlayerInstance);
            spawned = true;
        }

        GameManager.instance.MoveCamToPlayer(playerInstance);
    }
}
